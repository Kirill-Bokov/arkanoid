...
export class LevelManager {
  ...
  public loadLevel(level: LevelSchema, metrics: LayoutMetrics): void {
    for (let row = 0; row < level.rows; row++) {
      for (let col = 0; col < level.cols; col++) {
        const type = level.grid[row][col] as BrickType;

        if (type === "empty") continue;

        const x = col * metrics.brickWidth;
        const y = level.offsetY + row * metrics.brickHeight;
